...
The final hyperparameters selected by **caret** for each estimator can be found in the `tuningParam` folder.
...
This relies on the data in `data/GerberGreenLarimer_APSR.csv` and the plotting
function defined in `code/4-gotv_S_plot_fkt.R`.
...
To run the experiment and save the results, one should run:
```
